...
namespace BusinessIncentives.Common.ReferenceData
{
	public static class StaticData
	{
		...
		public static List<string> PunctuationSymbolList = "!|\"|#|%|&|'|(|)|*|,|-|.|/|:|;|?|@|[|\\|]|_|{|}|<|>".Split('|').ToList();

		public static List<string> IgnorableWordsExpressions = ignorableExpressions.Split('|').ToList();

		public static Dictionary<AssistanceOrigin, string> AssistanceOrigins {
			get 
			{
				return new Dictionary<AssistanceOrigin, string>
				{
					{ AssistanceOrigin.GrantsGov, "grants.gov" },
					{ AssistanceOrigin.SamGov, "sam.gov" },
					{ AssistanceOrigin.ChooseColorado, "choosecolorado.com" },
					{ AssistanceOrigin.BusinessIncentives , "Business Incentives" },

				};
			}
		}

		public static Dictionary<FundingLevel, string> FundingLevels
		{
			get
			{
				return new Dictionary<FundingLevel, string>
				{
					{ FundingLevel.City, FundingLevel.City.ToString() },
					{ FundingLevel.State, FundingLevel.State.ToString() },
					{ FundingLevel.County, FundingLevel.County.ToString() },
					{ FundingLevel.Region, FundingLevel.Region.ToString() },
					{ FundingLevel.Federal, FundingLevel.Federal.ToString() }
				};
			}
		}
	}
